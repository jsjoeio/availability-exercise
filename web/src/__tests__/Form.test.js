...
describe('Form component', () => {
  it('should let you type in your name', () => {
    ...
    const { rerender, getByPlaceholderText, getByDisplayValue } = render(<Form getName={fakeFn} isValid shouldResetName={false} />)
    ...
    rerender(<Form getName={fakeFn} isValid shouldResetName />)
  ...
  it('should show an error message if isValid is false', () => {
    const fakeFn = jest.fn()
    const { rerender, getByPlaceholderText, getByText } = render(<Form getName={fakeFn} isValid shouldResetName={false} />)
    const input = getByPlaceholderText('Darrell Silver')
    // Input value should be empty to start
    expect(input.value).toEqual('')
    // Update the props, re-render to the same container
    // Pretend as if the user clicked book without filling out name
    rerender(<Form getName={fakeFn} isValid={false} shouldResetName={false} />)
    // Input value should now be empty
